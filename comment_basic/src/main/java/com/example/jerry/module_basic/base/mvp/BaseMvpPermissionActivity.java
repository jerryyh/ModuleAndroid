...
import com.example.jerry.module_basic.dialog.MyAlertDialog;
...
public abstract class BaseMvpPermissionActivity<V extends IView, P extends IPresenter<V>> extends BaseMvpActivity<V, P> {
    ...
    public  String initView(int requestCode) {
        String message = "";
        if (requestCode== BaseMvpPermissionActivity.PERMISSION_CAMERA){
            //照相机权限
            message = getString(R.string.home_permission_camera);
        }else if(requestCode == BaseMvpPermissionActivity.PERMISSION_PHONE){
            //电话权限
            message = getString(R.string.home_permission_phone);
        }else if(requestCode == BaseMvpPermissionActivity.PERMISSION_STORAGE){
            //文件操作权限
